...
export function OrbitalHeroSection({
  ...
  // Keep the ref in sync with the latest props on every render, via a
  // layout effect rather than a direct mutation during render — refs are
  // for imperative reads (inside the rAF loop below), not something to
  // write while rendering. useLayoutEffect runs synchronously right after
  // render/before paint, so the render-time write's original guarantee
  // (the ref is current before anything else can read it) still holds.
  useLayoutEffect(() => {
    props.current = {
      planets, yearSeconds, trailYears, compress, maxTurns, planeSpread, eccentricity, alignToCourse, driftSpeed, apex,
      viewRadius, tilt, spin, roll, lead, focus, scrim, scrimStrength, starCount, glow, showOrbits, showSunTrack,
