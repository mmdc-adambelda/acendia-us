...
5. **Trial ends → real monthly charge**: fast-forward (or wait) past the corrected `trial_end` → Stripe auto-generates the first real invoice → `invoice.paid` fires → subscription flips to `active`, a `payments` row for the $499/mo (or $798/mo with add-on) appears, distinct from the earlier $199 setup-fee payment.
6. **PayPal subscription → webhook → active**: same pattern via PayPal sandbox buyer account — confirm whichever Plan structure was configured (trial-cycle setup fee, or estimate-only) actually charges what's expected before going live with real PayPal traffic.
7. **Wise setup fee → Mark Site Live → second invoice → admin confirms**: choose Wise at checkout → only the $199 setup reference is shown → confirm it in `/admin/payments` → subscription flips to `trialing` → use **Mark Site Live** → verify a NEW pending Wise payment appears for the monthly amount → confirm that one too → subscription flips to `active`.
8. **Failed payment → past_due**: use a Stripe test card that declines on renewal → `customer.subscription.updated`/`invoice.payment_failed` → subscription flips to `past_due`, client sees the billing banner + email.
9. **Cancellation**: cancel via the Stripe Customer Portal → `customer.subscription.deleted` → status `cancelled`.
10. **Admin-can / client-cannot**: confirm a `client`-role login gets redirected away from `/admin` (`requireRole` + RLS both block it); confirm `staff` can reach `/admin` but plan activation toggles are hidden/blocked (admin+ only, per RLS `plans_write_admin`).

---

## 14. Real Billing Schedule (added after initial Phase 2-5 build, per owner direction)

The original Phase 2 implementation charged the setup fee and first month together at checkout. The owner clarified the actual business process: **clients pay only the setup fee today; the monthly plan doesn't start billing until 14 days after their site actually goes live** (site build typically takes 2-3 business days from setup payment). This is implemented as:

- **`lib/billing.ts`** — single source of truth for the schedule: `POST_GOLIVE_BILLING_DELAY_DAYS = 14`, and a `DEFAULT_FIRST_BILLING_DELAY_DAYS = 19` estimate (used only until the real go-live date is known) used at checkout time before staff has recorded a real go-live date.
- **Stripe**: checkout is now `mode: "payment"` (one-time, setup fee only) instead of `mode: "subscription"`. The webhook creates the actual recurring `Subscription` afterward via the API with `trial_end` set to the estimate — Stripe charges nothing until that date, and the subscription reads `status: "trialing"` in the meantime. Staff correct the exact date via `updateStripeSubscriptionTrialEnd()` (no re-charge, no re-approval).
- **PayPal**: `start_time` on the created Subscription delays its first charge to the same estimate. Unlike Stripe, PayPal doesn't support moving this date after creation — see the owner-facing limitation note in `CLIENT-PORTAL-SETUP.md` Part 3 for the two documented ways to structure the PayPal Plan around this.
- **Wise**: checkout collects only the setup-fee pending payment. The monthly amount is a **second, separate** pending payment created by staff via "Mark Site Live" — Wise was already fully manual, so this just splits one manual invoice into two, timed correctly.
- **`websites.went_live_at`** (migration `0006`) + **`/admin/clients/[id]` → Mark Site Live** (`app/api/admin/subscriptions/mark-live/route.ts`) is the one action that turns every provider's checkout-time estimate into the real, contractual date.
- Plans were renamed **"SEO Package"** and **"Social Media Add-On"** (migration `0007`, and `0003` updated for fresh installs) per the owner's naming preference.
- `/pricing/` (`app/pricing/page.tsx`) was rebuilt to never render an empty "pricing is being finalized" state — a hardcoded `FALLBACK_PLANS` constant (mirroring the seed data exactly) renders whenever the live database fetch comes back empty, and a new "How billing works" section explains the 3-step timeline in plain language.
